    ...
    For i = 1 To UBound(arr)
        If arr(i) Like "???编号:*" Then
            ...
            brr(n, 0) = vv(arr(i), "\d{4}-\d{2}-\d{2}")
                If arr(i + 1) Like "*包*" And Not (arr(i + 2) Like "买家留言*") Then
                    brr(n, 1) = arr(i + 2)
                ElseIf arr(i + 2) Like "*包*" Then
                    brr(n, 1) = arr(i + 3)
                ElseIf arr(i + 3) Like "*包*" Then
                    brr(n, 1) = arr(i + 4)
                Else
                    brr(n, 1) = ""
                End If
            ...
            brr(n, 3) = vv(arr(i), "\d{5,}")
                If arr(i + 1) Like "*自动关闭*" Or arr(i + 1) Like "退款中" Then
                    brr(n, 4) = arr(i + 1)
                ElseIf arr(i + 1) Like "*主动关闭*" Then
                    brr(n, 5) = arr(i + 1)
                ElseIf arr(i + 1) Like "买家留言*" And arr(i + 2) Like "*自动关闭*" Then
                    brr(n, 4) = arr(i + 2)
                ElseIf arr(i + 1) Like "买家留言*" And arr(i + 2) Like "*主动关闭*" Then
                    brr(n, 5) = arr(i + 2)
                Else
                    brr(n, 4) = ""
                End If
        End If
        If arr(i) Like "收货地址：" Then
                '匹配省市
                brr(m, 6) = vva(arr(i + 3), "[一-龢]{2,} ", 0)
                brr(m, 7) = vva(arr(i + 3), "[一-龢]{2,} ", 1)
                '匹配姓名电话
'                brr(m, 6) = vva(arr(i + 1), "[^,]+", 0)
'                brr(m, 7) = arr(i + 2)
                m = m + 1
        ElseIf arr(i) Like "实付：*" Then
                brr(n, 8) = vva(arr(i), "\d{2,3}\.\d{2}", 0)
                n = n + 1
        End If
    Next
...
Private Sub 复制到新表省份市级()
ActiveSheet.Copy after:=ActiveSheet
Columns("A:f").Delete
Cells.EntireColumn.AutoFit
'ActiveSheet.Name = "筛选"
Rows(1).Insert
[a1:j1] = Array("时间", "简称", "状态", "编号", "退货", "主动关闭", "省份", "市级", "金额", "成本")
...
Private Sub 复制到新表姓名电话()
ActiveSheet.Copy after:=ActiveSheet
Columns("A:f").Delete
Cells.EntireColumn.AutoFit
'ActiveSheet.Name = "筛选"
Rows(1).Insert
[a1:j1] = Array("时间", "简称", "状态", "编号", "退货", "主动关闭", "姓名", "电话", "金额", "成本")
Rows(1).Interior.Color = 14277081
Rows(1).Font.Size = 13
Rows(1).Font.Bold = True
Columns(1).HorizontalAlignment = xlRight: Columns(3).HorizontalAlignment = xlCenter
...
For i = 1 To UBound(arr)
    If arr(i, 3) Like "*自动关闭" Then
        arr(i, 1) = "退款完"
        arr(i, 3) = "退成功"
    ElseIf arr(i, 3) = "退款中" Then
        arr(i, 1) = "退款中"
    ElseIf arr(i, 4) <> "" Then
        arr(i, 1) = "未付款"
    ElseIf arr(i, 1) Like "*编号" Then
